...
#### 乐观锁（Optimistic Lock）
乐观锁是一种并发控制策略，该策略认为数据的并发冲突很少发生，故在读取数据时不加锁，允许多个线程同时读同一条数据，但是在更新数据前检查数据的版本是否不一致，并在执行 UPDATE 时检查版本是否一致。如果请求更新的数据中的版本信息与原数据中的版本信息不匹配，则意味着有其他操作已经修改了数据，当前操作将被拒绝。然后在每次数据修改时，变化版本信息。
版本信息通常是一个版本号（`version`字段）或时间戳。
##### MybatisPlus 实现乐观锁
[乐观锁插件 | MyBatis-Plus](https://baomidou.com/plugins/optimistic-locker/)
1. SpringBoot 配置乐观锁拦截器。
2. 数据库表结构中添加 version 字段。
	```MySQL
	version int unsigned DEFAULT 1 COMMENT '版本号'
	```
3. 在实体类字段上添加 `@Version` 注解。
#### Token
