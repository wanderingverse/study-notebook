...
```
远程调用摄像头（需要先开启 ADB TCP 模式）并录像到 data 目录。
```C
Set shell = CreateObject("Wscript.Shell")
...
strCommand = "cmd /c scrcpy.exe --video-source=camera --camera-id=0 --camera-size=1920x1080 --camera-fps=30 --record=""./UserData/video_" & timestamp & ".mkv"""
