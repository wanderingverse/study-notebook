...
* 【强制】代码中的命名不能随意缩写，而是尽量使用完整的单词组合，力求见名知意，像注释一样传达意图。
	* 动作 + 方法 + 策略命名法：适用于**带有特定实现策略**的功能性方法
		* 动作：说明这个方法是用途。一般情况下是一个动词或动宾结构。
		* 方法/目标：说明这个动作是作用在哪种对象或问题上的。
		* 策略：说明使用的是哪种算法或技术手段来完成这个动作。
...
* 【强制】项目名统一使用大驼峰命名。
* 【强制】资源名统一使用全小写命名，各单词间如果需要分隔，统一使用**连接符-**。
...
* 【强制】删除方法用 delete 做前缀。
* 【强制】修改方法用 update 做前缀。
* 【强制】数据对象实体类用 BO 做后缀，完整类名的组成部分为：数据表名 + BO。
...
## 日期时间
- 【强制】表示日期和时间的完整格式化格式必须指定为："yyyy-MM-dd HH:mm:ss"，尤其注意大小写。
## 集合处理
...
- 【强制】库名与项目名如非必要，请保持一致。
- 【推荐】数据库时区推荐使用 UTC，后端和数据库时间保持一致。由前端转换时区。
...
- 【强制】在多表联合查询 SQL 中，必须为每个查询的字段添加别名。
# 工程结构
## 应用分层
